...
test('local relay permits only deployed daily and activity-week methods and retains caller authentication', async t => {
  ...
  assert.equal(await send('/v1/career/daily-desk'), 200);
  assert.equal(await send('/v1/career/activity-week'), 200);
  const body = JSON.stringify({date: '2026-09-24', caseId: 'the-cheap-share', choiceId: 'compare'});
  assert.equal(await send('/v1/career/daily-desk/complete', 'POST', body), 200);
  for (const [path, method] of [['/v1/career/daily-desk', 'PUT'], ['/v1/career/activity-week', 'POST'],
    ['/v1/career/daily-desk/complete', 'GET'], ['/v1/career/daily-desk/unknown', 'POST']]) {
    assert.equal(await send(path!, method!, method === 'GET' ? undefined : '{}'), 404);
  }
  assert.equal(calls.length, 3); assert.equal(calls[2]!.init.body, body);
