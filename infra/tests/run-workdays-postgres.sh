...
runtime_dir="$infra_dir/.workdays-runtime"
...
"$postgres_bin/pg_ctl" -D "$runtime_dir/data" -l "$runtime_dir/log" -o "-c listen_addresses='' -c unix_socket_directories='$runtime_dir/socket' -p 65455" -w start >/dev/null
started=1
for migration in "$infra_dir"/migrations/*.sql; do
 "$postgres_bin/psql" -X -v ON_ERROR_STOP=1 -h "$runtime_dir/socket" -p 65455 -U trimmy_daily_owner -d postgres -f "$migration" >/dev/null
done
