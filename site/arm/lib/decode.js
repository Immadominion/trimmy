...
function renderRule(data, ctx = {}) {
  ...
  let keeperAmount;
  if (verb === 1) {
    // A vault deposit settles in the vault's share token, not in FXRP.
    keeperAmount = `${fmtUnits(keeperFee, 6)} yield-vault shares`;
  } else {
    const token = ctx.knownTokens?.[buyTokenId] ?? `token #${buyTokenId}`;
    const decimals = ctx.tokenDecimals?.[buyTokenId];
    keeperAmount = Number.isInteger(decimals)
      ? `${fmtUnits(keeperFee, decimals)} ${token}`
      : `${keeperFee} base units of ${token}`;
  }
  out += `<div class="note">Whoever runs it for you earns ${esc(keeperAmount)} per run,
    paid out of the result. You never pay gas.</div>`;
  out += '</div>';
  return out;
}
